...
export class HttpVtuProvider implements VtuProvider {
  ...
  async purchaseAirtime(params: VtuPurchaseParams): Promise<VtuPurchaseResult> {
    ...
    try {
      const url = new URL('/purchase', this.baseUrl).toString();
      ...
      // Interpret provider response generically: success if HTTP-level response
      // included an 'ok' or 'success' flag, or else assume success when status exists.
